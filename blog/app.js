...
const app = express();
mongo.connect("mongodb://localhost:27017/blogDB")
...
app.get("/",async function(req,res){
  try {
    ...
    console.log(err);
    res.status(500).send("Error occurred while retrieving posts");
  }  
})
...
app.get("/posts/:postId",async function(req,res){
  try {
    ...
    let post= await Post.findOne({_id: requestedPost});
    res.render("post",{title: post.title,content: post.content});
  } catch (err) {
    console.log(err);
    res.status(500).send("Error occurred while retrieving post id");
  } 
})
